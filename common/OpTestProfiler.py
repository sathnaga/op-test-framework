...
class profilerThread(threading.Thread):
    ...
    def run(self):
        ...
        self.profile_output = os.path.join(self.conf.output, self.name)
        fd = open(self.profile_output, "w+")
        while True:
            if self.freq > 0:
                if self.env == 'sut':
...
class Profilers(object):
    def __init__(self, profiler_cmd_path=None, profiler_cmd=None):
        ...
        """
        self.conf = OpTestConfiguration.conf
        self.path = profiler_cmd_path if profiler_cmd_path else os.path.join(os.path.dirname(os.path.abspath(__file__)), 'profilers')
        if not os.path.isfile(self.path):
            log.warning("Check the profiler command path, given path is not valid: %s", self.path)
        if not os.path.isfile(self.parser_path):
